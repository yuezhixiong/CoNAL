...
class RandomScaleCrop(object):
    ...
    def __call__(self, img, label, depth, normal):
        ...
        img_ = F.interpolate(img[None, :, i:i + h, j:j + w], size=(
            height, width), mode='bilinear', align_corners=True).squeeze(0)
        label_ = F.interpolate(label[None, None, i:i + h, j:j + w],
                               size=(height, width), mode='nearest').squeeze(0).squeeze(0)
        depth_ = F.interpolate(
            depth[None, :, i:i + h, j:j + w], size=(height, width), mode='nearest').squeeze(0)
        normal_ = F.interpolate(normal[None, :, i:i + h, j:j + w], size=(
            height, width), mode='bilinear', align_corners=True).squeeze(0)
...
class NYU(Dataset):
    def __init__(self, root, mode='train', augmentation=False):
        ...
        random.seed(688)
        np.random.seed(688)
        data_len = len(fnmatch.filter(os.listdir(
            self.root + '/train/image'), '*.npy'))
        train_index = list(np.random.choice(
            np.arange(data_len), size=int(data_len*0.8), replace=False))
        ...
        if self.mode == 'train':
            ...
            data_len = len(fnmatch.filter(
                os.listdir(self.root + '/val/image'), '*.npy'))
    ...
    def __getitem__(self, i):
        ...
        return image.float(), semantic.long(), depth.float(), normal.float()
